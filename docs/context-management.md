...
- **2026-09-24 5 回目整理**: 1,541 行 → 155 行(直近 6 ブロック = 09-20 の 3 件 + 09-19 の 3 件を残し、それ以前〈08-22 〜 09-18 の 51 ブロック〉は `archive/pre-2026-09-18-jsonld-vrt-print.md` へ移動)。通例の 2 ブロックより多く残したのは、未完了の持ち越し(会見録テキスト版の公開後に 09-19 号の節①を当て直す)が 09-19 号のブロックにしか無かったため。検証 4 つ(行数 155 / 無損失 diff 空 / マーカー 6 + 51 = 57 / アーカイブがマーカーで始まる)がすべて通った。
