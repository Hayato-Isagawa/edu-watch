...
1. 切り出し位置を `grep -nE '^(#+ )?\*{0,2}直近の状態' .claude/state/active.md` で確認。**ブロックの見出し記法は 3 通り混在している**(`**直近の状態(...)**` / `# 直近の状態 (...)` / `## 直近の状態`)ので、1 つに決め打ちすると 0 件になる
2. 残すブロックの境界となる「次に古いセッション開始行」を `TRIM_LINE` とする(その行の **前** までを残す)。直近 2 ブロックを残すのが通例
   - **並行セッションが同じ active.md に書いていることがある。** 新しいブロックは冒頭に積まれるので、読み込みと書き戻しのあいだに割り込まれると相手のブロックを消す。行番号ではなくブロック見出しの文字列で切り出し位置を決め、書き戻す直前に mtime が変わっていないことを確かめる
