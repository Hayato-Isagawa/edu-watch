...
- [x] ソース別 RSS / HTML パーサー(第 1 層 + 第 2 層の計 10 本)
...
#### 3.2.5 日経 教育 — `nikkei-edu`

- 有料記事が多いため、タイトル + 概要のみ取得 → 有料マーク付きで表示する方針
...
1. **RSS ソースの URL 確定** — 各ソースの RSS 提供の有無を事前に確認する(予備調査必要)
2. **日経 教育の有料記事扱い** — タイトルのみ収集 or スキップ
3. **コミットボットの表示名** — `edu-watch-bot` で OK か、別名にするか
