...
## 追補(2026-04-30、本 PR の続き): nier の採用情報除外と同タイトル dedupe

mext クリーンアップ後の運用観察で、nier 側にも 2 種類のノイズが残っていることが判明:

1. **採用情報**: 「期間業務職員」「時間雇用職員」「募集は終了」を含むタイトルが計 8 件。教員・保護者の関心領域からは外れる事務系採用情報
2. **同タイトル重複**: NIER のトップページ HTML には複数の `<ul class="c-newslist">` セクションがあり、同じ記事を異なる href で再掲する構造。parser が href ベース dedupe しか持たず、「URL 違い・タイトル日付同じ」の重複が 11 件残っていた

本 ADR の方針を nier にも適用:

- **`src/lib/sources/nier.ts`** に `EXPLICIT_EXCLUDE_PATTERNS`(`期間業務職員` / `時間雇用職員` / `募集は終了`)を追加し、`isNierEducationRelevant(title)` で除外
- parser の dedupe を **href + (title + 日付)の二重キー** に強化(`seenUrls` と `seenTitleKeys` を併用)
- 既存データから採用情報 8 件 + 同 publishedAt 日 + 同タイトルの重複 11 件 = 計 19 件を一度きり削除する `scripts/clean-nier-noise.ts` を実行
...
mext 側のクリーンアップ(本 ADR 本体)と合わせて、累計 -53 件のデータ品質改善になった。
