...
- **原則として改変しない**(誤植修正と、下記の前方参照の追記は例外)
- 決定を覆す場合は新規 ADR を起こし、**両方の「状態」行に上書き関係を書く**
  - 新 ADR: `採用(NNNN の〜を上書き)`。何を上書きしたかを具体に書く(全部か、一部か)
  - 旧 ADR: 前方参照を 1 行足す。**本文は変えない** — 旧 ADR だけを開いた読者が、古い手順に従わずに済むようにするため
  - 過去に上書きされた ADR(0003 / 0007)には前方参照が無い。気づいた時点で足してよい
...
- 状態: 採用 / 採用(NNNN の〜を上書き) / 撤回(NNNN で上書き)
- 日付: YYYY-MM-DD
- 関連 PR: #N, #M
