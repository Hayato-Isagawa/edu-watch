...
- **決定そのものは改変しない。** 誤植修正・メタデータのバックフィル(`関連 PR: TBD` の解決など)・セキュリティ上の伏せ字・下記のリポジトリ外パスの伏せ字・下記の状態の追記は改変にあたらない。後続 ADR が本文の更新を決定事項として宣言することもある(0046 §決定(2) → 0040 §C-7)
- 決定を覆す(一部だけ上書きする場合を含む)ときは新規 ADR を起こし、**新旧どちらの状態にも上書き関係を書く**
  ...
  - **状態の置き場所は 2 通りある。** 63 本は `- 状態:` 行、4 本(0037 / 0038 / 0054 / 0057)は `## Status` 見出しの下。どちらでも同じことを書く
...
- **ADR には `memory rule N` / `ルール N` という、リポジトリ外の個人メモを指す番号参照が残っている(直近では 0064)。** 番号そのものは第三者が辿れない。上の「改変にあたらない」に言い換えは含まれないので、既存の ADR に残った番号はそのままにする(参照の多くは同じ行か同じファイルの参照リストに規範の内容が併記してあり、番号を辿れなくても趣旨は読める)。**新規 ADR では番号で参照しない** — 規範の内容を本文に書き切るか、リポジトリ内の文書を指す
- **リポジトリ外の個人パス(運営者のホーム配下の計画メモ等)も第三者は辿れない。** 番号参照と違いパスは伏せ字にできるので、**ADR でも** `<運営者ローカルの計画メモ>` のような placeholder に置き換えてよい(上の「改変にあたらない」に含める)。**新規 ADR では最初から書かない**
...
- 状態: 採用 / 採用(NNNN を上書き) / 撤回(NNNN で上書き) / 撤回(理由)
  (一部だけ上書きされた側は合成する。例: `採用 / 〈範囲〉は ADR NNNN で上書き`)
- 日付: YYYY-MM-DD
- 関連 PR: #N, #M
