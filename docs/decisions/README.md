...
- **決定そのものは改変しない。** 誤植修正・メタデータのバックフィル(`関連 PR: TBD` の解決など)・セキュリティ上の伏せ字・下記の状態の追記は改変にあたらない。後続 ADR が本文の更新を決定事項として宣言することもある(0046 §決定(2) → 0040 §C-7)
- 決定を覆す(一部だけ上書きする場合を含む)ときは新規 ADR を起こし、**新旧どちらの状態にも上書き関係を書く**
  ...
  - **旧 ADR 側の追記が未整備のものが複数ある**(0046 / 0054 は末尾の `## 更新` 節にしか書いていない)。1 本が複数の ADR に上書きされている場合もあるので、気づいた時点で必要なぶんだけ足してよい
