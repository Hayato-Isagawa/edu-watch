...
| §2.2: 1 週間のニュースから「これは覚えておくべき」を人手で選別 | frontmatter `sections[].articleIds` で記事 id を参照、`sections[].comment` で論点整理 |
| §7: `/digest/` `/digest/[yyyy-mm-dd]` | URL 設計のとおり実装 |
| §12 KPI: 100% 公開率 / §13: 編集負荷 30 分以内 | テンプレート化された frontmatter + Content Collection の型補完で編集を高速化 |
...
このゲートを通すことで、PRD §12 の KPI(100% 公開率)と §13 の設計(編集負荷 30 分以内)を維持しながら、edu-watch のブランド(信頼最優先、商業メディアと差別化)を構造で担保する。
...
PRD §12 と整合(公開曜日だけは PRD の「毎週金曜」から土曜へ改めた。PRD は v1.0 承認済みの記録なので書き換えず、現行の曜日は [digest-workflow.md](./digest-workflow.md)「執筆上の約束」が定める):
