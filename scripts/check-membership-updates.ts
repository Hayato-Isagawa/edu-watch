...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(
      r1.changed === 1,
      `changed should be 1 (only id 1111... flips), got ${r1.changed}`
    );
    ...
    assert(
      byId.get("nikkyo-2026-04-22-1111111111111111")?.requiresMembership ===
        true,
      "id 1111... should be flipped to true"
    );
    assert(
      byId.get("nikkyo-2026-04-22-2222222222222222")?.requiresMembership ===
        undefined,
      "id 2222... should remain undefined (not in updates)"
    );
    assert(
      byId.get("nikkyo-2026-04-22-3333333333333333")?.requiresMembership ===
        true,
    ...
    assert(
      r4.changed === 0 && r4.total === 0,
      "missing date file should be no-op"
    );
