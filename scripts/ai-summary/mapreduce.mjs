...
if (!values.slug || !values.section) {
  console.error(
    "Usage: mapreduce.mjs --slug <slug> --section <section> [--registry <path>] [--out-dir <dir>]"
  );
  process.exit(1);
}
...
const chunkRanges = Array.isArray(rangesRaw)
  ? rangesRaw
...
console.log(
  `[ranges] section=${values.section}, ${chunkRanges.length} chunks loaded from ${chunkRangesPath}`
);
...
console.log(
  `[parse] ${pages.length} pages parsed (PDF total=${totalPdfPages}), model=${MODEL}, num_ctx=${NUM_CTX}, ollama=${OLLAMA_BASE_URL}`
);
...
console.log(
  `[chunks] ${chunks.length} chunks, sizes=[${chunks.map((c) => c.chars).join(", ")}]`
);
...
for (let i = 0; i < chunks.length; i++) {
  const c = chunks[i];
  console.log(
    `[map ${i + 1}/${chunks.length}] pages ${c.startPage}-${c.endPage}, ${c.chars} chars`
  );
...
${c.text}`;
  ...
  console.log(
    `  done: ${r.elapsedSec.toFixed(1)}s, prompt=${r.promptEvalCount}t, eval=${r.evalCount}t`
  );
}

const coveredPagesText =
  chunks.length === 1
...
const reduceInput = mapResults
  .map(
    (m, i) =>
      `## Chunk ${i + 1} (pages ${m.chunk.startPage}-${m.chunk.endPage})\n\n${m.text}`
  )
  .join("\n\n---\n\n");
...
console.log(
  `[reduce] integrating ${mapResults.length} chunk summaries, prompt ${reducePrompt.length} chars`
);
const reduce = await callOllama(reducePrompt);
await fs.writeFile(path.join(sectionDir, "summary.md"), reduce.text);
console.log(
  `[reduce done] ${reduce.elapsedSec.toFixed(1)}s, prompt=${reduce.promptEvalCount}t, eval=${reduce.evalCount}t`
);
...
const metrics = {
  ...
  reduce: {
    ...
    tokensPerSec: reduce.evalDurationSec
      ? reduce.evalCount / reduce.evalDurationSec
      : 0,
  },
...
await fs.writeFile(
  path.join(sectionDir, "metrics.json"),
  JSON.stringify(metrics, null, 2)
);
console.log(
  `[done] total=${metrics.totalElapsedSec.toFixed(1)}s, calls=${metrics.totalLLMCalls}, output=${metrics.outputChars}chars`
);
