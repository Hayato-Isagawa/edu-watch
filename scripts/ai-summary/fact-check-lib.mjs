...
export function judgeStrict(facts, summaryText, chunkSources) {
  ...
  const details = facts.map((f) => {
    ...
    const rawChunkHit = rawChunkText
      ? f.patterns.some((re) => re.test(normChunk))
      : false;
    let judgment;
  ...
  return {
    ...
    llmHallucinated: details.filter((d) => d.judgment === "llm_hallucination")
      .length,
...
export function computeGate(present, missing, totalFacts) {
  ...
  const status =
    missingHigh.length > 0 ? "BLOCK" : missing.length > 0 ? "WARN" : "PASS";
  return {
    status,
...
export function buildRetryPrompt(summary, missingFacts, chunkSources) {
  const missingList = missingFacts
    .map((f, i) => `${i + 1}. ${f.retryHint || `必須データ点 ${i + 1}`}`)
    .join("\n");
