...
if (!values.slug) {
  console.error(
    "Usage: run-pipeline.mjs --slug <slug> [--section <section>] [--skip-retry] [--skip-extract] [--skip-mapreduce] [--registry <path>] [--out-dir <dir>] [--pdf <path>] [--url <url>]"
  );
  process.exit(1);
}
...
if (!values["skip-extract"]) {
  ...
  else if (entry.pdfUrl && entry.pdfUrl !== "TBD")
    extractArgs.push("--url", entry.pdfUrl);
  else {
    console.error(
      "extract requires --pdf, --url, or non-TBD pdfUrl in registry"
    );
    process.exit(1);
  }
...
for (const section of targetSections) {
  ...
  const factCheckExit = await runStepCapture(
    "fact-check-grep.mjs",
    factCheckArgs
  );
...
for (const r of sectionResults) {
  ...
  console.log(
    `  ${r.section}: ${r.elapsedSec.toFixed(1)}s, canonical ${present}/${total}, gate=${gateStatus} (out-of-scope ${outOfScope})`
  );
}
