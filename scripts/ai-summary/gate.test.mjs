...
test("judgeStrict: rawChunk に在り canonical 本体に無い fact は llm_dropped", () => {
  const facts = [
    { id: "x", severity: "HIGH", patterns: [/10:06/], sourceChunks: [1] },
  ];
  const canonicalSummary =
    "本体要約には高校教諭の在校等時間の具体値が含まれない。";
  const chunkSources = {
    1: "高等学校教諭の10月・11月の平日在校等時間は 10:06 である。",
  };
...
test("buildRetryPrompt: retryHint 使用時、答案数値が不足リストに漏れない", () => {
  ...
  assert.ok(
    prompt.includes("高等学校教諭の10・11月の平日・土日の在校等時間"),
    "retryHint が含まれていない"
  );
});
...
test("buildRetryPrompt: retryHint 欠落でもフォールバックが answer 数値・id を漏らさない", () => {
  ...
  const prompt = buildRetryPrompt("本体要約（数値なし）", facts, {});
  assert.ok(
    !prompt.includes("10:06"),
    "フォールバックで答案数値 10:06 が漏れている"
  );
  assert.ok(
    !prompt.includes("1006"),
    "フォールバックで id 由来の数値 1006 が漏れている"
  );
    ...
    entries: [
      {
        slug: "fixture",
        sections: [
          { section: "s1", chunkRanges: "cr.json", requiredFacts: "rf.json" },
        ],
      },
    ],
  };
...
test("統合: HIGH/MEDIUM 全充足で fact-check が exit 0（PASS）", () => {
  const { tmp, outDir } = setupFixture(
    "HIGHマーカー と MEDIUMマーカー を含む完全な要約。"
  );
  const res = runFactCheck(tmp, outDir);
  rmSync(tmp, { recursive: true, force: true });
