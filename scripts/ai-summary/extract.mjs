...
if (!values.slug) {
  console.error(
    "Usage: extract.mjs --slug <slug> (--pdf <path> | --url <url>) [--registry <path>] [--out-dir <dir>]"
  );
  process.exit(1);
}
...
console.log(
  `[output] extracted-full.txt: ${text.length} chars (all ${pages.length} pages)`
);

for (const section of entry.sections) {
  const { startPage, endPage } = resolveBoundary(
    section.boundary,
    pages.length
  );
  if (startPage > endPage || startPage < 1 || endPage > pages.length) {
    console.error(
      `[error] section ${section.section}: invalid range ${startPage}-${endPage} (total ${pages.length})`
    );
    process.exit(1);
  }
  ...
  console.log(
    `[output] extracted-${section.section}.txt: ${body.length} chars (pages ${startPage}-${endPage})`
  );
}
