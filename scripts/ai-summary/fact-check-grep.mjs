...
if (!values.slug || !values.section) {
  console.error(
    "Usage: fact-check-grep.mjs --slug <slug> --section <section> [--skip-retry] [--registry <path>] [--out-dir <dir>]"
  );
  process.exit(1);
}
...
if (!section) {
  console.error(
    `section "${values.section}" not found in slug "${values.slug}"`
  );
  process.exit(1);
}

const baseDir = path.resolve(
  values["out-dir"] || `./tmp/ai-summary/${values.slug}`
);
...
const SYSTEM =
  "あなたは日本の文部科学省が発出する公文書を読み、現場の学校教員にとって有益な要約を作成する編集者です。事実誤認は絶対に避け、文書に書かれていないことは書かないでください。数値・固有名詞・時期はすべて原文と一致させてください。";
...
async function loadRawChunkSources(chunkNumbers) {
  ...
  const ranges = Array.isArray(rangesRaw)
    ? rangesRaw
    : rangesRaw[values.section];
  if (!Array.isArray(ranges)) {
    throw new Error(
      `chunkRanges must be array (or {section: array}) in ${chunkRangesPath}`
    );
  }
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const end =
      i + 1 < matches.length ? matches[i + 1].index : extracted.length;
  ...
  for (const n of chunkNumbers) {
    ...
    if (!r) {
      throw new Error(
        `chunk ${n} not found in ${chunkRangesPath} (length=${ranges.length})`
      );
    }
    const parts = [];
...
async function loadAllRawChunkSources(facts) {
  const allChunkNumbers = [
    ...new Set(facts.flatMap((f) => f.sourceChunks || [])),
  ].sort((a, b) => a - b);
...
console.log(
  `[facts] loaded ${facts.length} required facts from ${requiredFactsPath}`
);
...
console.log(
  `[grep] present ${present.length}/${facts.length}, missing ${missing.length} (ollama=${OLLAMA_BASE_URL})`
);
for (const m of missing) {
  console.log(
    `  [missing] ${m.severity} ${m.id}: ${m.description} (sourceChunks=[${(m.sourceChunks || []).join(", ")}])`
  );
}

const recoverable = missing.filter(
  (f) => f.sourceChunks && f.sourceChunks.length > 0
);
...
if (recoverable.length === 0) {
  ...
  const chunkNumbers = [
    ...new Set(recoverable.flatMap((f) => f.sourceChunks)),
  ].sort((a, b) => a - b);
  const allChunkSources = await loadRawChunkSources(chunkNumbers);
  console.log(
    `[retry] missing ${recoverable.length}, chunks [${chunkNumbers.join(", ")}], per-chunk retry (raw)`
  );
  ...
  for (const n of chunkNumbers) {
    ...
    const chunkOutputPath = path.join(
      sectionDir,
      `summary-checked-raw.chunk${n}.md`
    );
    await fs.writeFile(chunkOutputPath, result.text);
    console.log(
      `[retry chunk ${n} done] ${result.elapsedSec.toFixed(1)}s, output ${result.text.length} chars → ${chunkOutputPath}`
    );
    const grepAfter = grepFacts(result.text, facts);
    const chunkRecovered = recoverable.filter((f) =>
      grepAfter.find((g) => g.id === f.id && g.found)
    );
...
const report = {
  ...
  outOfScope: outOfScope.map((f) => ({
    ...summarizeFact(f),
    reason:
      "missing-in-chunk (chunk 段階から脱落、ADR 0040 §C-6 編集者最終監修で補完)",
  })),
};
...
console.log(
  `[gate] ${gate.status} — canonical present ${present.length}/${facts.length}, missing HIGH ${gate.missingHigh.length} / MEDIUM ${gate.missingMedium.length}`
);
console.log(
  `[retry/参考] recovered ${recovered.length}, still-missing ${stillMissing.length}, out-of-scope ${outOfScope.length}（advisory・本体未統合）`
);
console.log(
  `[strict/参考] present ${strict.strictPresent}/${strict.totalFacts}, llm_hallucination ${strict.llmHallucinated}, llm_dropped ${strict.llmDropped}, missing ${strict.stillMissingStrict}`
);
