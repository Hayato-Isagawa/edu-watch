...
function run(f, input) {
  return spawnSync("/bin/bash", [f.hook], {
    cwd: f.shared,
    encoding: "utf8",
    input,
  });
...
test("malformed JSON on stdin + worktree path in it -> falls back -> blocked", () => {
  withFixture((f) => {
    const r = run(
      f,
      `{"tool_input": {"file_path": "${path.join(f.wt, "README.md")}"`
    );
    assert.equal(r.status, 2, r.stderr);
    assert.match(r.stderr, /BLOCKED: editing on 'main'/);
  });
...
test("path outside any git repo -> falls back to the hook's own checkout (main) -> blocked", () => {
  withFixture((f) => {
    ...
    const r = spawnSync("git", ["-C", outside, "rev-parse", "--git-dir"], {
      encoding: "utf8",
    });
    assert.notEqual(
      r.status,
      0,
      "fixture: outside dir must not be inside a git repo"
    );
